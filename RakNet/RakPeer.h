...
//	unsigned int FirstFreeRemoteSystemLookupIndex(const SystemAddress &sa) const;
	...
	std::string preauthKey;
	bool isRbxConnection = false;
	...
	virtual bool RunUpdateCycle( RakNet::TimeUS timeUS, RakNet::Time timeMS, BitStream &updateBitStream );
	...
	static std::pair<unsigned, RBX::Network::Rupp::Rupp::RuppInfo> ReadRuppHeader(const char* data, const int length, RakPeer* rakPeer, RakNet::TimeUS timeUS);

	static void ProcessOfflineNetworkPacket(SystemAddress systemAddress, const char* data, const int length, RakPeer* rakPeer, RakNetSmartPtr<RakNetSocket> rakNetSocket, RakNet::TimeUS timeRead, RakNet::TimeUS timeUS, unsigned short ruppLength, std::string_view gameServiceToken);
	void sendRbxOpenError(unsigned char packetId, SystemAddress systemAddress, RakNetSmartPtr<RakNetSocket> rakNetSocket, RakNet::TimeUS timeUS, bool ruppFromServer);
	bool processRbxOpenRequest1(const char* data, const int length, SystemAddress systemAddress, RakNetSmartPtr<RakNetSocket> rakNetSocket, unsigned short ruppLength, RakNet::TimeUS timeUS);
	void sendRbxOpenReply1(const int totalLength, SystemAddress systemAddress, RakNetSmartPtr<RakNetSocket> rakNetSocket, RakNet::TimeUS timeUS, bool ruppFromServer);
	bool processRbxOpenReply1(const char* data, const int length, SystemAddress systemAddress, RakNetSmartPtr<RakNetSocket> rakNetSocket);
	bool processRbxOpenRequest2(const char* data, const int length, SystemAddress systemAddress, RakNetSmartPtr<RakNetSocket> rakNetSocket, unsigned short ruppLength, std::string_view gameServiceToken, RakNet::TimeUS timeUS);
	bool processRbxOpenReply2(const char* data, const int length, SystemAddress systemAddress, RakNetSmartPtr<RakNetSocket> rakNetSocket, RakNet::TimeUS timeRead);
	enum class PreauthResult
	{
		FailToVerify,
		VerifiedFirst,
		VerifiedSecond,
	...
	PreauthResult verifyPreauthMac(std::string_view preauthBlob, std::uint8_t authVersion);

	static bool ProcessBannedPacket(SystemAddress systemAddress, RakPeer* rakPeer, RakNetSmartPtr<RakNet::RakNetSocket> rakNetSocket, BitStream& updateBitStream, RakNet::TimeUS timeUS);
	void ProcessBufferedCommands(BitStream& updateBitStream, RakNet::TimeUS timeUS, RakNet::TimeMS timeMS);
	void ProcessBufferedPackets(BitStream& updateBitStream, RakNet::TimeUS timeUS);
	void HandleRequestedConnectionQueue(RakNet::TimeUS timeUS, RakNet::TimeMS timeMS);
	void HandleActiveSystemList(BitStream& updateBitStream, RakNet::TimeUS timeUS, RakNet::TimeMS timeMS);
